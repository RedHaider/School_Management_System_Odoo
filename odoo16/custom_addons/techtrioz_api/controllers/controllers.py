...
from odoo.addons.techtrioz_api.common import invalid_response, valid_response, convert_data_str
...
class TechtriozApi(http.Controller):
...
# Haider's Manufacture get
    @validate_token
    @http.route("/api/get_manufacturing_orders", type="http", auth="none", methods=["GET"], csrf=False)
    def get_manufacturing_orders(self, **payload):
        """ NOTE: DEPRECATED API for now, Gets the specific time frame from request and returns product details."""

        manufacturing_orders = request.env['mrp.production'].sudo().search([])

        # Extract relevant data from manufacturing orders
        orders_data = []
        ...
        return Response(json.dumps(orders_data), content_type='application/json;charset=utf-8')
        
## Haider's Manufacture get ends
    ...
    def _get_dashboard_values(self):
        ...
        res.update({"wms_licensing_key": request.env['ir.config_parameter'].sudo().get_param(
            'techtrioz_api.wms_licensing_key') or ""})
    ...
    @http.route('/get_manufacturing_orders',type="http", auth="none", methods=["GET"], csrf=False)
    def get_manufacturing_orders(self, **payload):
        # Assuming your manufacturing order model is named 'mrp.production'
        manufacturing_orders = request.env['mrp.production'].sudo().search([])
    ...
    def _get_stock_quants(self, payload_data):
        ...
        if 'location_barcode' in payload_data or 'product_id' in payload_data or \
                'package_name' in payload_data or 'lot/serial_number' in payload_data:
            ...
            if 'lot/serial_number' in payload_data:
                if payload_data['lot/serial_number']:
                    stock_quant_domain = stock_quant_domain + \
                        [('lot_id.name', '=', payload_data['lot/serial_number'])]
            if is_admin == 0:
                stock_quant_domain = stock_quant_domain + \
                ...
                for move_line in move_line_ids:
                    if move_line.get('id') is None:
                        ...
                        if move_line.get('lot_id'):
                            line_domain.append(
                                ('lot_id', '=', stock_prod_lot.search([('name', '=', move_line.get('lot_id'))]).id))
